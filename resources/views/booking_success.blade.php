...
<div class="bg-white p-10 rounded-3xl shadow-xl max-w-lg w-full text-center border border-gray-100">
    <div class="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
        <i class="fas fa-check text-4xl text-green-600"></i>
    </div>

    <h1 class="text-3xl font-black text-gray-800 uppercase tracking-tighter mb-2">Booking Confirmed!</h1>
    <p class="text-gray-500 mb-8 text-sm font-medium">Thank you, {{ $booking->first_name }}. Your reservation has been successfully processed.</p>
    ...
    <div class="bg-gray-50 rounded-2xl p-6 text-left border border-gray-100 mb-8">
        ...
        <div class="space-y-3">
            <div class="flex justify-between text-sm">
                <span class="text-gray-500 font-semibold">Vehicle:</span>
                <span class="font-bold text-gray-800">{{ $booking->vehicle->title ?? 'N/A' }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-gray-500 font-semibold">Pick-up:</span>
                <span class="font-bold text-gray-800">{{ \Carbon\Carbon::parse($booking->pickup_date)->format('d M, Y') }} at {{ $booking->pickup_time }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-gray-500 font-semibold">Drop-off:</span>
                <span class="font-bold text-gray-800">{{ \Carbon\Carbon::parse($booking->dropoff_date)->format('d M, Y') }} at {{ $booking->dropoff_time }}</span>
            </div>
    ...
    <p class="text-[11px] text-gray-400 mb-8 leading-relaxed">
    ...
    <div class="flex flex-col sm:flex-row gap-4">
        <a href="/" class="flex-1 bg-orange-600 text-white font-black py-4 rounded-xl hover:bg-orange-700 transition text-center uppercase text-xs tracking-widest shadow-md shadow-orange-100">
            Back to Home
        </a>

        @if(isset($stripeInvoiceUrl))
            <a href="{{ $stripeInvoiceUrl }}" target="_blank" class="flex-1 bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition shadow-md text-center uppercase text-xs tracking-widest flex items-center justify-center gap-2">
    ...
    <div class="bg-gray-50 rounded-2xl p-4 sm:p-6 text-left border border-gray-100 mb-6 sm:mb-8">
        ...
        <div class="space-y-3">
            ...
            <div class="flex justify-between gap-4 text-sm border-t border-gray-200 pt-3 mt-3">
                <span class="text-gray-500 font-semibold shrink-0">Total Price:</span>
                <span class="font-bold text-gray-800 text-right">€{{ number_format($booking->total_price - ($booking->insurance->deposit_price ?? 0), 2) }}</span>
            </div>
        </div>
    </div>
