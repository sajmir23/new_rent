                    ...
                    <div class="card">
                        <div class="card-header border-info border-3 d-inline-flex flex-column flex-md-row align-items-center justify-content-between">
                            ...
                            <div class="btn-group" role="group" aria-label="Actions">
                                <a href="{{ route('admin.dashboard') }}" class="btn btn-primary" title="All Notifications">
                            ...
                            <div class="table-responsive">
                                <table class="table align-middle table-row-dashed fs-6 gy-5" id="notifications-table">
                                    ...
                                    @foreach($notifications as $notification)
                                        <tr  @if(is_null($notification->read_at))  @endif  >
                                            <td>
                                                @if(is_null($notification->read_at))
                                                    <form method="POST" action="{{ route('admin.notifications.markAsRead', $notification->id) }}">
                                                        @csrf
                                                        <button type="submit" class="btn btn-bg-light btn-active-color-primary btn-sm me-1 ">
