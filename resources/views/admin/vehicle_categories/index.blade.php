            ...
            <div class="d-flex flex-column flex-row-fluid">
                <div class="d-flex align-items-center pt-1">
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold">
                        ...
                        <li class="breadcrumb-item text-white fw-bold lh-1">Vehicle Categories List</li>
                    </ul>
                </div>
                <div class="d-flex flex-stack flex-wrap flex-lg-nowrap gap-4 gap-lg-10 pt-13 pb-6">
                    <div class="page-title me-5">
                        <h1 class="page-heading d-flex text-white fw-bold fs-2 flex-column justify-content-center my-0">Vehicle Categories List</h1>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div id="kt_app_content" class="app-content flex-column-fluid">
                    <div class="card">
                        <div class="card-header border-info border-3 d-inline-flex flex-column flex-md-row align-items-center justify-content-between">
                            <h4 class="card-title">Vehicle Categories List</h4>
                            <div class="btn-group" role="group" aria-label="">

                                @if(auth()->user()->hasPermission('vehicle_categories.store'))
                                <a href="{{route('admin.vehicle_categories.create')}}" id="new_content" class="btn btn-primary mr-1" title="Clear Filter">
                                    <i class="fa fa-plus"></i>New Vehicle Category</a>
                                @endif

                                <a href="javascript:void(0);" id="datatable-reset-filter" class="btn btn-warning mr-1" title="Clear Filter">
                                    <i class="fa fa-broom"></i>Clear Filter</a>
                            </div>
                        </div>
    ...
    <script>
        ...
        function confirmDeleteAction(){
            ...
            $.ajax({
                ...
                url: '{{route("admin.vehicle_categories.destroy")}}',
            }).done(function (data) {
    ...
    <script>
        $(document).ready(function () {
            ...
            let dt = $datatable.DataTable({
                ...
                order: [[1, 'asc']],
                ajax: {
                    url: '{{ route('admin.vehicle_categories.index') }}',
                    data: function (d) {
                        d.title     = $('#title').val();
                    }
                },
