{{--<div id="tab-car-content" class="modal-tab">
    <div class="flex justify-between items-start mb-6">
        <h2 class="text-4xl font-black text-gray-900 uppercase tracking-tighter">{{ $vehicle->title }}</h2>
        <button class="text-gray-300 hover:text-red-500 transition"><i class="far fa-heart text-2xl"></i></button>
    </div>

    <div id="imageModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-90 flex items-center justify-center p-4">
        <span class="absolute top-5 right-10 text-white text-4xl cursor-pointer hover:text-orange-500" onclick="closeModal()">&times;</span>
    ...
    <div class="grid grid-cols-4 gap-3 mb-8">
        <div class="col-span-4 lg:col-span-3">
            <img id="mainImage"
                 ...
                 class="w-full h-80 object-cover rounded-3xl shadow-sm transition-opacity duration-300 cursor-zoom-in">
        </div>
    ...
    <div class="mb-10">
        ...
        <div class="grid grid-cols-2 gap-y-4 border-t border-gray-50 pt-4">
            <div class="flex justify-between pr-6 border-r border-gray-50"><span class="text-gray-400">Gear box</span> <span class="font-bold uppercase text-xs">{{ $vehicle->transmissionType->title_en }}</span></div>
            <div class="flex justify-between pl-6"><span class="text-gray-400">Engine</span> <span class="font-bold uppercase text-xs">{{ $vehicle->engine_size }} L</span></div>
            <div class="flex justify-between pr-6 border-r border-gray-50"><span class="text-gray-400">Year</span> <span class="font-bold uppercase text-xs">{{ $vehicle->year }}</span></div>
            <div class="flex justify-between pl-6"><span class="text-gray-400">Fuel</span> <span class="font-bold uppercase text-xs">{{ $vehicle->fuelType->title_en }}</span></div>
        </div>

        <div id="all-specs" class="hidden mt-4 space-y-4 pt-4 border-t border-dotted border-gray-200">
            <div class="grid grid-cols-2 gap-y-4">
                <div class="flex justify-between pr-6 border-r border-gray-50"><span class="text-gray-400">Seats</span> <span class="font-bold">{{ $vehicle->seats }}</span></div>
                <div class="flex justify-between pl-6"><span class="text-gray-400">Drive</span> <span class="font-bold uppercase text-xs">AWD</span></div>
                <div class="flex justify-between pr-6 border-r border-gray-50"><span class="text-gray-400">Air Conditioning</span> <span class="font-bold">Yes</span></div>
                <div class="flex justify-between pl-6"><span class="text-gray-400">Cruise Control</span> <span class="font-bold">Yes</span></div>
            </div>
        </div>
    ...
    <div class="mb-10">
        ...
        <div class="grid grid-cols-1 gap-4">
            @forelse($insurances as $index => $insurance)
                <label class="relative flex items-start p-5 rounded-2xl border-2 {{ $loop->first ? 'border-orange-500 bg-orange-50/30' : 'border-gray-100 hover:border-orange-200' }} transition cursor-pointer insurance-label">

                    <input type="radio"
                           name="insurance"
                           value="{{ $insurance->id }}"
                           {{ $loop->first ? 'checked' : '' }}
                           class="mt-1 h-5 w-5 accent-orange-600 insurance-radio"
    ...
    --}}{{--<div class="mb-4">
        ...
        <div class="space-y-3">
            @forelse($additionalServices as $service)
                <div class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl hover:shadow-sm transition cursor-pointer" onclick="this.querySelector('input').click()">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" class="h-5 w-5 accent-orange-600 rounded additional-service-checkbox"
                               data-id="{{ $service->id }}"
                               value="{{ $service->id }}"
    ...
    <div class="mb-4">
        ...
        <div class="space-y-3">
            @forelse($additionalServices as $service)
                <div class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl hover:shadow-sm transition">
                    <div class="flex items-center gap-3">
                        <span class="text-sm font-semibold text-gray-700">{{ $service->title_en ?? $service->name }}</span>
                    </div>
...
<div id="tab-booking-content" class="modal-tab hidden">
    <h2 class="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-8">Booking Details</h2>

    <div class="grid grid-cols-2 gap-6 mb-10">
        <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100">
            <span class="text-[10px] font-black text-orange-600 uppercase mb-2 block tracking-widest">Pick-up</span>
            <div id="display-pickup-loc" class="font-bold text-gray-800 text-sm mb-1">...</div>
        ...
        <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100">
            ...
            <div id="display-dropoff-loc" class="font-bold text-gray-800 text-sm mb-1">...</div>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" id="first_name" placeholder="First Name *" class="p-4 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-500">
            <input type="text" id="last_name" placeholder="Last Name *" class="p-4 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-500">

            <input type="date" id="birthday" placeholder="Date of birth *" class="p-4 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-500">
            <input type="email" id="email" placeholder="Email *" class="p-4 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-500">
            <input type="tel" id="phone" placeholder="Contact phone number *" class="p-4 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-500">
        </div>
...
{{--<div id="summary-content">
    <h3 class="text-xl font-black text-gray-800 uppercase tracking-tight mb-8">Cost Summary</h3>

    <div class="space-y-4 mb-8 border-b border-gray-100 pb-8">
        <div class="flex justify-between text-sm">
            <span class="text-gray-400">Rent for {{ $totalDays }} days</span>
            <span class="font-black text-gray-800">€{{ number_format($vehicle->base_daily_rate * $totalDays, 2) }}</span>
        </div>

        <div class="flex justify-between text-sm">
            <span class="text-gray-400">Insurance</span>
            <span class="font-black text-gray-800">Included</span>
        </div>
        <div class="flex justify-between text-sm">
            <span class="text-gray-400">Additional services</span>
            <span class="font-black text-gray-800">As selected</span>
        </div>
    </div>

    <div class="bg-orange-50 p-6 rounded-[2rem] border border-orange-100 mb-8">
        <div class="flex justify-between items-center">
            <span class="text-[10px] font-black text-orange-900 uppercase tracking-widest">Total Estimated</span>
            <span class="text-3xl font-black text-orange-600">€{{ number_format(($vehicle->base_daily_rate * $totalDays), 2) }}</span>
        </div>
    </div>
    ...
    <div class="mb-8">
        ...
        <div class="space-y-3">
            ...
            <!-- Stripe -->
            <label class="flex items-center justify-between p-4 border border-gray-200 rounded-2xl cursor-pointer hover:border-orange-400 transition">
                <div class="flex items-center gap-4">
            ...
            <label class="flex items-center justify-between p-4 border border-gray-200 rounded-2xl cursor-pointer hover:border-orange-400 transition bg-white">
                <div class="flex items-center gap-4">
                    <input type="radio" name="payment_gateway" value="pok" class="w-4 h-4 accent-orange-600">

                    <div class="flex items-center justify-center p-2 rounded-lg -ml-4" style="width: 80px; height: 50px;">
    ...
    <button id="modal-main-btn" class="w-full bg-orange-600 text-white font-black py-5 rounded-[1.5rem] hover:bg-orange-700 transition shadow-lg shadow-orange-100 uppercase text-xs tracking-widest flex items-center justify-center gap-2">
        <span>Continue</span> <i class="fas fa-arrow-right text-[10px]"></i>
    </button>
</div>--}} {{--Me pak e rendesishme kjo pjese e kodit--}}

{{--<div id="summary-content">

    <input type="hidden" id="calc-total-days" value="{{ $totalDays }}">
    <input type="hidden" id="calc-base-rent" value="{{ $vehicle->base_daily_rate * $totalDays }}">
    <h3 class="text-xl font-black text-gray-800 uppercase tracking-tight mb-8">Cost Summary</h3>
    ...
    <div class="space-y-4 mb-8 border-b border-gray-100 pb-8">
        ...
        <div class="flex justify-between text-sm">
            <span class="text-gray-400">Insurance</span>
            <span id="summary-insurance" class="font-black text-gray-800">Included</span>
        </div>

        <div class="flex justify-between text-sm">
            <span class="text-gray-400">Additional services</span>
            <span id="summary-services" class="font-black text-gray-800">€0.00</span>
        </div>
