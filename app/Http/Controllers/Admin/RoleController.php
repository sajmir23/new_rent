...
class RoleController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if (! auth()->user()->hasPermission('roles.view_any')) {
            $this->forbiddenLogService->storeForbiddenLog( request()->path() , 'roles.view_any','Can not view All roles');
        ...
        return view('admin.roles_permissions.roles')->with([
            'roles'              => $roles,
    ...
    public function store(Request $request){
        ...
        if (! auth()->user()->hasPermission('roles.store')) {
            $this->forbiddenLogService->storeForbiddenLog( request()->path() , 'roles.store','Can not store new role');
            return view('admin.errors.unauthorized');
        }
        ...
        try {
            ...
            return  redirect()->route('admin.roles.index')->with('successMessage', __('general.stored_successfully'));
    ...
    public function managePermissions(Request $request,$id){
        ...
        if (! auth()->user()->hasPermission('roles.manage_permissions')) {
            $this->forbiddenLogService->storeForbiddenLog( request()->path() , 'roles.manage_permissions','Can not manage role permissions');
            return view('admin.errors.unauthorized');
        }

        $role             = Role::findOrFail($id);
        $role_permissions = DB::table('permission_role')->where('role_id',$id)->pluck('permission_id')->toArray();
        $permissions      = Permission::query()->where('scope', 'system')->get();
        ...
        return view('admin.roles_permissions.manage_permissions')->with([
            'role'                => $role,
            'groupedPermissions'  => $groupedPermissions,
    ...
    public function update(Request $request,$id){
        ...
        if (! auth()->user()->hasPermission('roles.update')) {
            $this->forbiddenLogService->storeForbiddenLog( request()->path() , 'roles.update','Can not update role');
            return view('admin.errors.unauthorized');
        }
