...
class InsuranceExpiryReminder extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ...
            ->line("Insurance Expiring: {$this->vehicle->insurance_expiry->format('d-m-Y')} ({$this->daysLeft} days left)")
    ...
    {
        $expiryDate = $this->vehicle->insurance_expiry
            ? $this->vehicle->insurance_expiry->format('d-m-Y')
            : 'N/A';

        return [
        'message' => "The vehicle'{$this->vehicle->title}' with plate '{$this->vehicle->plate}' has its insurance expiring on {$expiryDate}."
        ];
    }
}
